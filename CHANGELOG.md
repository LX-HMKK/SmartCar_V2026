...
- `nav_only.yaml` 与 `default_waypoints.yaml` 均保持 `calibrated: false`。两份路线共享三段
  拓扑、方向、goal profile 与除 A 外的几何；语义 QR/A 点和受看护 P→A 停点单独维护。
- 新增固定、显式的受看护纯导航前缀，只允许在人工看护下复验 P→A 或 P→C1；它不自动
  解除急停、发车或授权完整路线。

### 现场记录与边界

- 2026-08-04 在 RDK `172.16.24.170` 的受看护纯导航动作均报告成功，但 P→A 观察到车辆越过
  物理 QR/A 点后才完成判定。该结果不能作为当前坐标、倒车、媒体任务或完整赛道验收证据。
...
- 移除全部导航行为树的 `OdomDistanceBudget` 人工里程、里程计超时和步长熔断器，并删除对应
  BT 插件、构建目标与测试；路线可达性、碰撞和终点判断继续由 Nav2 planner、controller、costmap
  与 goal checker 负责。
- 方向门不再对倒车 `angular.z` 二次取反；反向 MPPI 虚拟前向映射已包含正确的倒车转向关系。
- 任务适配层不再把 EKF `/odom` 静止阈值作为动作终态或新租约的前置条件；方向门 `Stop` 成功
  撤销租约后即可切换下一导航段。
- `ReverseHandoff` 保持 `3.0 s` 预测窗，并将采样批量降至 `100`；相同参数同步到 Gazebo
  覆盖配置，避免 RDK X5 A55 上的控制循环饥饿。

### 验证

- 本机 Gazebo `nav_only` 完整三段通过：P→A `13.62 s`、A→C1 `31.29 s`、C1→P `35.07 s`。
- RDK `172.16.24.66` 完成同一路线纯导航实车运行：P→A `12.06 s`、A→C1 `30.95 s`、
  C1→P `32.26 s`，终态 `mission_completed` 并回到 P 点。第二段的一次 costmap 新鲜度失败由
  已有清图重规划恢复；未发生控制器掉线、命令超时或段间中止。
- Nav2/仿真合同测试 `94` 项通过；`smartcar_task` 包测试与任务 launch 合同 `80` 项通过。
