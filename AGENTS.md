...
> 同步基线：2026-08-01。`AGENTS.md` 与 `CLAUDE.md` 除工具名称外应保持一致。
## 项目背景与当前状态
...
截至 2026-08-01，本机 Ubuntu 22.04 直接运行 Ignition Gazebo 6.18 仿真，不使用
WSL。Gazebo/RViz/TF/Nav2 启动，P→A 与反向规划不可达后的 `0.15 m` 短退、清图和
重规划已有运行证据。`AckermannReverseRetreat` 只在反向树中经 `FollowPath` 执行，先做
global/local raw costmap 的完整足迹扫掠，拒绝过期、越界或碰撞的盲退。当前全路线仍未
通过：第二段恢复后 `b_corridor_enter` 距离为 `0.533 m`，超过 `0.50 m` transit 阈值。
该证据只覆盖 Gazebo 模型，不属于实体倒车或现场验收。
...
- **导航层**：单一 Smac Hybrid `DUBIN` planner + Regulated Pure Pursuit；自由过渡点由 `ComputeFreeHeadingPathAction` 解析，不要求到达 yaw。倒车多航点使用虚拟航向与反向校验；规划候选在清图后仍穷尽时，`AckermannReverseRetreat` 可通过专用 `ReverseRecovery` controller 退让一次 `0.15 m` 后重规划。当前仿真路线为 P→A 单航点 forward、其余四段 reverse ThroughPoses；不启动 `behavior_server` 或 `waypoint_follower`，禁止 Spin recovery 和原地旋转。
...
- 当前仿真 `nav_only.yaml` 的方向模式为：P→A 单航点 `forward`，其后的 `reverse_corridor`、`reverse_c_entry`、`c_exit` 和 `return_to_p` 均为 `reverse`；多航点阶段必须使用 ThroughPoses。`validate_waypoints()` 必须拒绝全正向或越界方向配置。
...
当前保存的 all-reverse 路线已验证 Gazebo/RViz/TF/Nav2 启动、P→A 与 A→B 规划失败时的
安全短退/重规划。第二段当前仍因 `b_corridor_enter` 误差 `0.533 m > 0.50 m` 未通过；
全程不应标注为通过。离线预检的下一候选是将非任务点 `b_corridor_enter` 从
`(1.10, 2.85)` 下移至 `(1.10, 2.65)`，必须以完整结果 JSON 复验。
...
- 修正并验证仿真 `b_corridor_enter -> c_corner_1` 倒车入口与 C 区中央 keepout 的安全余量；推荐先验证 `b_corridor_enter=(1.10,2.65)`。P、QR、VLM 为保护任务点；其他点位可按几何预检调整位置，不强制到达朝向。
- 🔴 当前 all-reverse 路线尚未到达 `c_corner_4 -> b_corridor_return_enter`；不得将旧正向路线的 yaw 调参结果视为当前倒车路线的通过证据。
...
- 🔴 **NavigateThroughPoses 路线与倒车 (2026-08-01)**：路线为五阶段——① P→A 单航点 forward；② A→B reverse ThroughPoses；③ B→C3（经 C1/C2）reverse ThroughPoses；④ C3→return_enter（经 C4）reverse ThroughPoses；⑤ return_enter→P（经 b_corridor_return）reverse ThroughPoses。非任务点由 free-heading 搜索处理，不使用 `REEDS_SHEPP`、`reverse_penalty` 或航点朝向翻转。若候选搜索在清图后仍穷尽，反向树最多安全短退一次再重规划。该恢复已在 Gazebo 触发，但当前第二段仍未满足 `b_corridor_enter` 的 transit 位置合同，必须先验证推荐 B 点调整并取得完整结果 JSON。
...
- ℹ️ **仿真航点几何扫描 (2026-08-01)**：`src/smartcar_sim/scripts/geometry_scan.py` 对任意航点段做 Dubins 路径扫描（R=0.55m），采样起姿包络、检查 B/C 区 keepout、墙碰撞和 corridor 可达性。输出 JSON 候选集供 `apply_candidate.py` 消费；写入后必须以完整结果 JSON 验收。
