...
> 面向智慧医疗赛的全自动智能车系统：车辆自 **P** 起点发车，途经二维码观察点 **A** 与诊疗区角点 **C1**，在 A 点读取二维码、在 C1 点执行图生文人物描述，最终返回 **P** 终点。全程由 Nav2 基于实时 costmap 自主规划，以 Aurora 深度相机为唯一视觉与障碍感知来源（RGB 模块供图像识别、深度模块供动态避障），融合 IMU/轮式里程计定位，完成动态避障与语义任务。

### ✨ 系统亮点

- **全自动** —— 全正向路线自主导航，任务状态机编排二维码 / 图生文任务
...
| 📷 深度相机 | Aurora 930（随 OriginCar 底盘附带） | **RGB 模块**供二维码/图生文视觉输入，**深度模块**供动态障碍感知 | USB |
...
| 🖼️ **图生文任务（C1 点）** | VLM 只描述 C1 点实际拍摄到的图像，30 s 为请求上限；空结果或请求失败会明确结束语义任务，不生成通用猜测。 |
...
- 支持二维码、图生文两个独立媒体入口与一键系统启动，可按现场需要单独启用。

### 工程验证状态

- ✅ 本机 Gazebo 已完成纯导航全路线软件校验。
- ⏳ 深度相机动态障碍感知（costmap 标记 / 清障与实时重规划）、二维码 / 图生文语义任务与受看护现场跑动仍需逐项验收。

## 六、运行约束与优化方向

### 已知运行约束

- **完整任务时长** —— 一次完整的 `P → A → C1 → P` 运行约为 `60 s`；任务按此预算设计，而非追求速度纪录。
- **深度点云仅 10 Hz** —— Aurora 深度点云是响应速度的上限来源。Nav2 避障链无法响应得比这更快，因此速度上限设为 `0.5 m/s`。
- **转弯时避障很紧张** —— 转弯后点云帧才刷新，转弯后冒出的锥桶要等车身贴近时才出现点云信息，因此避障总是反复触发 `BackUp` + 重规划，而不是提前干净地避过。

### 优化方向（规划中，未进入当前版本）

1. **用 YOLO + PnP 识别锥桶替代深度点云** —— 用 YOLO 识别锥桶，通过 PnP 把障碍距离信息投影进 costmap，替代深度模块的障碍输入。
2. **低负载任务不停车** —— 二维码任务可在 A 区持续识别（同步优化点位），到达 B 区通道时再根据二维码结果决定 C 区路径，无需停车。VLM 任务同理无需停车：拍取画面后后台处理即可，反正裁判是等车回到 **P** 点才查看 UI 信息。
3. **直接淘汰深度点云** —— Aurora 的 **RGB** 模块不错（帧率高、广角）；点云会扫到外围挡、误触发避障，导致无法准确回到 **P** 点。弃用点云，保留 RGB。
