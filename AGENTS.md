...
- 经过点（`task: via`、`through_ids`）只作为 Nav2 `ComputePathThroughPoses` 的有序路径规划约束，明确拒绝参与到达判定、停车、任务完成或分段终点。`via` 不得作为 `end_id`，`FollowPath` 的到达检测只能针对该段非 `via` 的最终端点。每段正常情况下只允许在开始时基于实时 costmap 计算一次完整路径，随后单次 `FollowPath` 跟踪该路径，期间不得重算、重投或重新施加已通过的 `via` 约束。唯一例外是路径跟踪失败或不可达后的原生 Nav2 脱困：最多一次直线 `BackUp`（`0.20 m`、速度不超过 `0.15 m/s`），随后 `RemovePassedGoals` 仅删除已经通过的规划约束并重算当前段一次；它不构成到达、停车或任务完成，也不得连续恢复。
...
- 当前全正向树禁止 Spin、Wait 和 `DriveOnHeading`。每个导航动作只允许一次受限的原生 Nav2 `BackUp` 脱困（`0.20 m`、不超过 `0.15 m/s`），不允许反向航点、反向规划或反向到达判定；只有经过点树会在脱困后用 `RemovePassedGoals` 清理已通过的规划约束，`REVERSE` 租约不是任务导航路径的一部分。方向门拒绝超限或带转向的回退命令并记录警告，但不得因此锁存停车或撤销前进任务许可。
