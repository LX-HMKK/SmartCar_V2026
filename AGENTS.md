...
- **NavigateThroughPoses 三阶段路线 (2026-07-27)**：路线从逐航点 `NavigateToPose` 升级为三阶段 ThroughPoses，减少 BT 重规划开销和交接延迟。阶段划分：① QR 单航点 forward（`navigate_to_pose_w_replanning_and_recovery.xml`）；② corridor 2 点 reverse ThroughPoses（`navigate_through_poses_reverse_w_replanning_and_recovery.xml`，使用 `ComputeReversePathThroughPoses`）；③ C 区 loop+return 6 点 forward ThroughPoses（`navigate_through_poses_w_replanning_and_recovery.xml`）。`nav_only.yaml` 移除 `b_corridor_out`（10→9 nav 航点），`b_corridor_return_enter` 经 `geometry_scan.py` 重定位至 (2.3, 2.5, -120°)。`auto_train.py` 按三阶段分段发送（`use_through_poses:=true`），fallback 退化为逐航点。已知问题：reverse BT XML 曲率校验 `curvature_tolerance=0.30`（默认 0.25 不足）；forward ThroughPoses 约 60s 后规划失败，仍在调试。C 区循环点（c_corner_1~4）位置不可改，仅可改 yaw；B 区走廊点（b_corridor_*）可上延至 C 区边缘（允许占一点 C 区）。P/QR/VLM 点位不得移动。调参流程：`geometry_scan.py` 枚举候选 yaw → `apply_candidate.py` 写入 YAML → `sim_tune.sh --loop 1` 验证。
