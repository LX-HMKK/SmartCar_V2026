...
- 转向、轮速、陀螺仪标定尚未完成。
- 端侧 VLM 模型文件尚未部署，默认 `vlm_backend_mode=disabled`。
...
`smartcar_bringup` 的 launch test 使用合成 `/scan`、`/odom`、`/odom_combined` 和静态 TF，关闭底盘、LiDAR、障碍物驱动和实体相机，并在 Nav2 激活前锁存急停。它验证 Nav2 lifecycle、vision/task 服务、任务 `IDLE`、硬件节点隔离和 `/cmd_vel_safe` 精确全零：
...
关键输出：`/cmd_vel_safe`、`/smartcar/safety/status`、`/smartcar/task/state`、`/smartcar/vision/read_qr`、`/smartcar/vision/describe_scene`。
