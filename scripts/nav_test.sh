...
# 用法: bash /home/sunrise/ros2_ws/scripts/nav_test.sh [--go] [--wheel-only]
...
for arg in "$@"; do
  case "$arg" in
    ...
    *)
      echo "未知选项: $arg"
      echo "用法: bash /home/sunrise/ros2_ws/scripts/nav_test.sh [--go] [--wheel-only]"
      exit 1
      ;;
  esac
done
...
nohup ros2 launch smartcar_bringup smartcar_system.launch.py \
  ...
  use_task:=true \
  $EXTRA_ARGS $LOCALIZATION_PROFILE_ARG use_visualization:=true waypoints_file:="$WP" \
  >> "$LOG" 2>&1 &
LAUNCH_PID=$!
