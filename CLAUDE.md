...
- 经过点（`task: via`、`through_ids`）只作为 Nav2 `ComputePathThroughPoses` 的有序路径规划约束，明确拒绝参与到达判定、停车、任务完成或分段终点。`via` 不得作为 `end_id`，`FollowPath` 的到达检测只能针对该段非 `via` 的最终端点。每段正常情况下只允许在开始时基于实时 costmap 计算一次完整路径，随后先执行上述原生实时平滑，再由单次 `FollowPath` 跟踪其结果；平滑可在实时 costmap 和碰撞约束下自然切过 `via` 的边缘，但不改变其规划约束或到达语义。跟踪期间 `RemovePassedGoals` 只持久清理已通过的队首规划约束，不参与到达判定。唯一例外是路径跟踪失败或不可达后的原生 Nav2 脱困：每段最多 6 次直线 `BackUp`（每次 `0.20 m`、速度不超过 `0.25 m/s`），每次随后只用剩余规划约束重算当前段并重新执行上述实时平滑；它不构成到达、停车或任务完成，也不重投已清理的 `via`。`RemovePassedGoals` 的半径仅是 Nav2 通用队首裁剪规则，不保证拓扑意义上的通过。
...
- EKF 的 yaw 输出完全来自 IMU 的 z 轴角速度：轮式合成的 yaw 不可靠且被 EKF 忽略（`odom0_config` 角速度为 `false`），标定后的 IMU yaw 可靠。
...
- 当前全正向树禁止 Spin、Wait 和 `DriveOnHeading`。每个导航动作最多允许 6 次受限的原生 Nav2 `BackUp` 脱困（每次 `0.20 m`、不超过 `0.25 m/s`），不允许反向航点、反向规划或反向到达判定；经过点树在跟踪和每次脱困后都用 `RemovePassedGoals` 持久清理已通过的队首规划约束，`REVERSE` 租约不是任务导航路径的一部分。方向门拒绝超限或带转向的回退命令并记录警告，但不得因此锁存停车或撤销前进任务许可。
- 当前全正向纯导航路线已完成本机 Gazebo 全链软件校验，具备基础通行性。不得因 YAML 的 `calibrated: false`、默认运动门禁为 `false` 或缺少重复文档记录而将该路线再表述为“未验收”，也不得以此拒绝已获本次明确授权的受看护深度测试。
...
- 本机 Gazebo 纯导航已完成当前路线的全链软件校验；这不替代 QR/VLM 语义任务验收，也不替代深度相机的动态障碍感知验证。
- 发车时必须人工将车辆放在 P 原点，车头朝 `+X`；任务 reset 不能替代物理复位。

## 工作方式

- 先读取相关代码、当前 YAML 和 [`docs/README.md`](docs/README.md) 中对应的文档。
- 本地 `src/`（含各包内 `config/`）与全局 `config/`（DDS/udev）是权威源。`push` 默认使用 `--delete`，RDK 有本地修改时必须先 pull 或备份。
- 默认验证命令：`python3 -m unittest discover -s tests -v`。详细仿真、RDK 构建、急停与恢复步骤见部署手册。
- 本文件与 `AGENTS.md` 保持一致：两者是同一套约束，改任一处须同步；合同测试以 `AGENTS.md` 为准。
