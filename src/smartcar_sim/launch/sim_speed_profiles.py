...
This module is imported only by ``sim.launch.py`` and produces a temporary
overlay layered after the simulation keepout configuration when an explicit
speed trial is requested.
"""
...
def _speed_parameters(document: Mapping[str, Any]) -> tuple[float, list[Any], list[Any]]:
    ...
    try:
        ...
        desired = float(follow_path["desired_linear_vel"])
        maximum = smoother["max_velocity"]
        minimum = smoother["min_velocity"]
    except (KeyError, TypeError, ValueError) as error:
        raise ValueError(
            "sim speed overlay must set FollowPath.desired_linear_vel and "
    ...
    return desired, maximum, minimum
...
def validate_speed_overlay(profile: SimSpeedProfile, overlay_path: Path) -> None:
    ...
    if set(controller_parameters) != {"FollowPath"} or set(smoother_parameters) != {
        "max_velocity",
        "min_velocity",
    }:
        raise ValueError(
    ...
    follow_path = controller_parameters.get("FollowPath")
    if not isinstance(follow_path, Mapping) or set(follow_path) != {
        "desired_linear_vel"
    }:
        raise ValueError(
            "sim speed overlay may only change FollowPath.desired_linear_vel"
        )

    desired, maximum, minimum = _speed_parameters(document)
    expected = profile.linear_speed_mps
    if (
        len(maximum) != 3
        or len(minimum) != 3
        or desired != expected
        or [float(value) for value in maximum] != [expected, 0.0, 0.75]
        or [float(value) for value in minimum] != [-expected, 0.0, -0.75]
    ):
        raise ValueError(
